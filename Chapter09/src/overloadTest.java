 ...
 * 넓이, 부피 계산하는 부분은 메소드로 작성하시오.
 * 사용자가 엔터키를 누르면 입력을 종료하고, 
 * 종료전에 입력한 숫자의 개수에 따라 도형의 결과 값을 출력합니다.
 * 
 * [문제분석]
 * 	입력받은 정수의 개수에 따라 도형의 넓이를 계산하는 메소드를 오버라이딩하여 생성한다.
 * 	입력받는 값은 엔터키를 누를 때 까지 입력한 수의 개수를 배열에 저장한다.
 *  배열에 저장된 길이에 따라 해당 메소드를 호출한다.
 */
import java.util.Scanner;

class Overload{
	public void calc(){   // 메소드 오버로딩
		System.out.println("매개 변수가 없습니다.");
	}
	
	public void calc(int r){   // 메소드 오버로딩
		System.out.println("원의 넓이 : " + r * r * 3.14);
	}
	
	public void calc(int width, int height){   // 메소드 오버로딩
		System.out.println("직사각형의 넓이 : " + width * height);
	}
	
	public void calc(int width, int height, int depth){   // 메소드 오버로딩
	...
	public static void main(String args[]){
        ...
        while (true) {
            ...
            switch (shape_len.length) {
                case 1:
                    ol.calc(Integer.parseInt(shape_len[0]));
                    break;
                case 2:
                    ol.calc(Integer.parseInt(shape_len[0]), Integer.parseInt(shape_len[1]));
                    break;
                case 3:
                    ol.calc(Integer.parseInt(shape_len[0]), Integer.parseInt(shape_len[1]), Integer.parseInt(shape_len[2]));
                    break;
                default:
